...
#Инициализация либ
pygame.init()
pygame.mixer.init(channels=12)

#Класс игры
class Game:
	def __init__(self):
		#Гл. Настройки
		self.version = "0.1v" #Версия игры
		self.MASHTAB = 4 #Умножение размера текстур (Норма - 5. Нетбуки - 4)
		...
		self.CONSOLE_ENABLED = 1 #Отвечает за разблокировку консоли игры. Обычно включается во время разработки или в дев. билдах (auto-py-to-exe)
	...
	def Values(self):
		...
		self.POLICE_CAR_X = 0 #Координата полицейской тачки по оси X
		self.POLICE_CAR_Z = 0 #Координата полицейской тачки по оси Z
...
# and self.PLAYER_CAR_Y == self.CAR1_Z
	def ColliderOfMachines(self):
		...
		#self.POLICE_CAR_X = self.PLAYER_CAR_X
	...
	def TestPolygon(self):
		...
		while self.running:
			...
			#self.screen.blit(self.POLICE_VAZ_PNG, (self.POLICE_CAR_X, self.POLICE_CAR_Z))
