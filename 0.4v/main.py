...
class Game:
	def __init__(self):
		...
		self.GAME_OVER_SCREEN = pygame.image.load("assets/Images/GAME_OVER_SCREEN.png")
		...
		self.GAME_PAUSE = pygame.mixer.Sound("assets/GFX/GAME_PAUSE.ogg")
		#Game Font
...
# and self.PLAYER_CAR_Y == self.CAR1_Z
	...
	def MainMenu(self):
		...
		while True:
			...
			self.TextDraw(f"By AltTeam. TG:t.me/lytigonchic", 	2 * self.MASHTAB, (255, 255, 255), (0, self.screensize[1] - 2 * self.MASHTAB), self.screen)
	...
	def Pause(self):
		...
		self.TextDraw(f"Пауза", 3 * self.MASHTAB, (255, 255, 255), (60 * self.MASHTAB, 8.4 * self.MASHTAB), self.screen)
		self.TextDraw(f"Выйти из паузы - ENTER", 3 * self.MASHTAB, (255, 255, 255), (34.6 * self.MASHTAB, 90 * self.MASHTAB), self.screen)
		while True:
			KEYS=pygame.key.get_pressed()
	...
	def GameOverScreen(self):
		...
		while self.running:
			#self.screen.fill((255, 255, 255)) 4 170
			self.AnimatedBackground()
			self.MachinePosGenerate()
			self.screen.blit(self.PLAYER_CAR_SRFC, (self.PLAYER_CAR_X, self.PLAYER_CAR_Y))
			if self.GAME_TYPE == "2P":
				self.screen.blit(self.PLAYER2_CAR_SRFC, (self.PLAYER2_CAR_X, self.PLAYER2_CAR_Z))
			self.screen.blit(self.GAME_OVER_SCREEN, (0, 26 * self.MASHTAB))
			self.TextDraw(f"Game Over!", 3*4 * self.MASHTAB, (0, 0, 0), (10.5 * self.MASHTAB, 30 * self.MASHTAB), self.screen)
			self.TextDraw(f"Ваши очки:{self.PLAYER_SCORE}", 3 * self.MASHTAB, (0, 0, 0), (46 * self.MASHTAB, 47 * self.MASHTAB), self.screen)
			self.TextDraw(f"Рекорд:{self.MAX_SCORE}", 3 * self.MASHTAB, (0, 0, 0), (46 * self.MASHTAB, 43.5 * self.MASHTAB), self.screen)
			self.TextDraw(f'Нажмите "Enter" для начала новой игры.', 3 * self.MASHTAB, (0, 0, 0), (11.5 * self.MASHTAB, 64 * self.MASHTAB), self.screen)
